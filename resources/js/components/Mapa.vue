<template>
    <div id="mapContainer" class="basemap child-component"></div>
</template>
  
<script>
import mapboxgl from "mapbox-gl";
import MapboxGeocoder from '@mapbox/mapbox-gl-geocoder';
import '@mapbox/mapbox-gl-geocoder/dist/mapbox-gl-geocoder.css';

export default {
    name: "BaseMap",
    /* 
    
    
    Añadir prop de componente padre APP para pasar la dirección el incidente y marcarlo con un marker en el mapa, 
    de momento creo una variable para poder trabajar en el prototipo llamada direccionIncidente, 
    luego se pasará un prop con este nombre
    
    
    */
    props: ['seleccion', 'send', 'restartJson'],
    data() {
        return {
            accessToken: 'pk.eyJ1Ijoiam5pcmVsbGFtcG9saXRlY25pY3MiLCJhIjoiY2xlc2ZkN2tiMDRrYjNzbnpmcGh1eG1payJ9.BYMy0fBa_so0Iz-qJMwGkA',
            localizaciones: [],
            localizaciones1: [],
            localizaciones2: [],
            localizaciones3: [],
            localizaciones4: [],
            localizaciones5: [],
            localizaciones6: [],
            localizaciones6: [],
            lat: 2.18215, //2.24631 santfost
            lang: 41.4021, //41.51561 santfost
            booleanPrueba: false,
            mapa:null,
            /*
            
            
            Eliminar esta variable cuando se cree el prop de direccionIncidente


            */
            direccionIncidente: "Sant fost de campsentelles, calle osona 20",
            incidentGeocoder: [],
            sendVar: false
        };
    },
    //Watchers
    watch: {
        //cuando se le da a confirmar al seleccionar tipo de agencias del formulario padre
        sendVar: function (newVal, oldVal) {
            console.log('send watcher')
            
            if (newVal) {
                this.obtenerAgencias(this.seleccion);
            }
           
        },
        //cuando se cierra el modal del mapa grande se resetean las localizaciones seleccionadas
        restartJson: function (newVal, oldVal) {
            this.localizaciones = [],
                this.localizaciones1 = [],
                this.localizaciones2 = [],
                this.localizaciones3 = [],
                this.localizaciones4 = [],
                this.localizaciones5 = [],
                this.localizaciones6 = [],
                this.localizaciones7 = []
        },
        //Centrar mapa en la incidencia
        lat: function (newVal, oldVal) {
            let map= this.mapa
            alert (this.lat)
            map.setCenter([this.lat,this.lang]);
        }
    },
    //Metodos
    methods: {
        //Se obtienen de la BBDD la info de las agencias seleccionadas por tipo
        obtenerAgencias(arrayId) {
            if (arrayId.length > 1) {
                arrayId.forEach(id => {
                    axios
                        .get('/Broggitecnics/public/api/agencies/' + id)
                        .then(response => {
                            const data = this.localizaciones
                            response.data.data.forEach(element => {
                                data.push(
                                    element
                                )
                            })
                            return new Promise(resolve => resolve());
                        })
                        .then(() => {
                            return this.marcasAgencias();
                        });
                });
            } else {
                axios
                    .get('/Broggitecnics/public/api/agencies/' + arrayId)
                    .then(response => {
                        const data = this.localizaciones
                        response.data.data.forEach(element => {
                            data.push(
                                element
                            )
                        })
                        console.log('localizaciones')
                        console.log(this.localizaciones)

                        //promise para que no se ejecute el method marcasAgencias asincronamente
                        return new Promise(resolve => resolve());

                    })
                    .then(() => {
                        return this.marcasAgencias();
                    });
            }

        },
        

        //Metodo para obtener GeoJsons de las localizaciones e imprimir los marcadores personalizados
        async marcasAgencias() {
            let me = this;

            //CARGA MAPA CENTRADO EN LAS COORDENADAS QUE SE CALCULAN ASYNCRONAMENTE PERO NO SE PUEDE TRABAJAR EN EL MAPA ACCENDIENDO A LOS METODOS DEL OBJETO:
            function getMapData(me) {

                return new Promise((resolve, reject) => {
                    axios
                        .get(
                            "https://api.mapbox.com/geocoding/v5/mapbox.places/" + me.direccionIncidente + ".json?access_token=" + me.accessToken)
                        .then((response) => {
                            if (!response) {
                                console.error("Invalid response:");
                                console.error(response);
                                reject("Invalid response");
                            } else {
                                console.log("INCIDENT");
                                console.log(response);
                                console.log(
                                    "Respuesta OK NOMBRE: " + response.data.features[0].place_name
                                );
                                console.log("Coordenadas: " + response.data.features[0].center);

                                me.lat = parseFloat(response.data.features[0].center[0]);
                                me.lang = parseFloat(response.data.features[0].center[1]);
                                
                                //guardar todas las localizaciones con sus coordenadas creadas con forwardGeocode en JSON
                                let newData = {
                                    type: "Feature",
                                    properties: {},
                                    geometry: {
                                        type: "Point",
                                        coordinates: response.data.features[0].center,
                                    },
                                };

                                let lugar = me.incidentGeocoder;
                                lugar.push(newData);

                                resolve({ lat: me.lat, lang: me.lang });
                            }
                        })
                        .catch((error) => {
                            console.error(error);
                            reject(error);
                        });
                });
            }

            //Actualizar latitud y longitud en variable global del componente
            getMapData(me).then(({ lat, lang }) => {
                me.lat=lat
                me.lang=lang
            });












            

            const map = new mapboxgl.Map({
                container: "mapContainer",
                style: "mapbox://styles/mapbox/streets-v11",
                center: [2.2001107, 41.406612],
                zoom: 12,
            });
            me.mapa= map
            

            //CREAR DISTINTOS GEOJSONS PARA CADA TIPO DE AGENCIA Y IMPRIMIR MARCADORES PERSONALIZADOS
            await Promise.all(this.localizaciones.map(async localizacion => {
                //si el codigo postal tiene solo 4 digitos, se le añade un 0 delante (está mal en la base de datos)
                let codiPostal = localizacion.codi_postal
                if (codiPostal.toString().length == 4) {
                    codiPostal = '0' + codiPostal.toString()
                } else if (codiPostal.toString().length < 4) {
                    alert('DANGER!! el codigo postal tiene menos de 4 digitos')
                }

                let direccion = codiPostal + ", " + localizacion.carrer
                axios
                    .get("https://api.mapbox.com/geocoding/v5/mapbox.places/" + direccion + ".json?access_token=" + me.accessToken)
                    .then((response) => {
                        if (!response) {

                            console.error('Invalid response:');
                            console.error(response);
                            return;
                        }
                        else {
                            console.log(response);
                            console.log('Respuesta OK NOMBRE: ' + response.data.features[0].place_name);
                            console.log('Coordenadas: ' + response.data.features[0].center);

                            //guardar todas las localizaciones con sus coordenadas creadas con forwardGeocode en JSON
                            let newData = {
                                type: 'Feature',
                                geometry: {
                                    type: 'Point',
                                    coordinates: response.data.features[0].center
                                },
                                properties: {
                                    title: localizacion,
                                    description: response.data.features[0].place_name,
                                    nombre: localizacion.nom,
                                    tipoAgencia: localizacion.AgenciesPrimaries_id
                                }
                            };

                            console.log('NewData')
                            console.log(newData)

                            //si se recibe más de una seleccion, se guardan en jsons distintos para poder asignarles luego un marcador personalizado
                            let id = localizacion.AgenciesPrimaries_id;
                            this.seleccion.forEach(element => {
                                let seleccion;
                                switch (id) {
                                    case 1:
                                        seleccion = this.localizaciones1
                                        break;
                                    case 2:
                                        seleccion = this.localizaciones2
                                        break;
                                    case 3:
                                        seleccion = this.localizaciones3
                                        break;
                                    case 4:
                                        seleccion = this.localizaciones4
                                        break;
                                    case 5:
                                        seleccion = this.localizaciones5
                                        break;
                                    case 6:
                                        seleccion = this.localizaciones6
                                        break;
                                    case 7:
                                        seleccion = this.localizaciones7
                                        break;
                                }
                                seleccion.push(newData);
                            })
                        }

                    });

            }));

            // NO PUEDO ACCEDER AL OBJETO MAP
            map.on('load', function () {


                /*


                PREGUNTAR:


                NO PUEDO USAR ESTA FUNCION PORQUE ME DA ERROR
                me.cargarImagenMarcador('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSFauoqR0cwPDZulkn-nGOaGbwjwDwmr4Z8Sg&usqp=CAU','mossos')
                CODIGO REPETIDO MANUALMENTE: 



                */



                //Cargar imagen de mossos
                map.loadImage(
                    'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSFauoqR0cwPDZulkn-nGOaGbwjwDwmr4Z8Sg&usqp=CAU',
                    (error, image) => {
                        if (error) throw error;

                        // Add the image to the map style.
                        map.addImage('mossos', image);

                        //CREO QUE ESTO NO ES NECESARIO, CARGA LA IMAGEN IGUALMENTE

                        // Add a data source containing one point feature.
                        // map.addSource('pointMossos', {
                        //     'type': 'geojson',
                        //     'data': {
                        //         'type': 'FeatureCollection',
                        //         'features': [
                        //             {
                        //                 'type': 'Feature',
                        //                 'geometry': {
                        //                     'type': 'Point',
                        //                     'coordinates': [-77.4144, 25.0759]
                        //                 }
                        //             }
                        //         ]
                        //     }
                        // });
                    })

                //Cargar imagen transito
                map.loadImage(
                    'data:image/jpeg;base64,',
                    (error, image) => {
                        if (error) throw error;

                        // Add the image to the map style.
                        map.addImage('transit', image);
                    })

                //Cargar imagen bomberos
                map.loadImage(
                    'data:image/png;base64,',
                    (error, image) => {
                        if (error) throw error;

                        // Add the image to the map style.
                        map.addImage('bombers', image);
                    })

                //Cargar imagen policia local
                map.loadImage(
                    'data:image/png;base64,',
                    (error, image) => {
                        if (error) throw error;

                        // Add the image to the map style.
                        map.addImage('pLocal', image);
                    })

                //Cargar imagen policia municipal
                map.loadImage(
                    'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRpGMuq1MLSmXrRoI2ItobTgsQZo2lyNRO80g&usqp=CAU',
                    (error, image) => {
                        if (error) throw error;

                        // Add the image to the map style.
                        map.addImage('pMunicipal', image);
                    })

                //Cargar imagen Guardia Urbana
                map.loadImage(
                    'data:image/png;base64,',
                    (error, image) => {
                        if (error) throw error;

                        // Add the image to the map style.
                        map.addImage('gUrbana', image);
                    })



                //Cargar imagen incidente
                map.loadImage(
                    'https://static.vecteezy.com/system/resources/previews/009/663/747/original/warning-icon-transparent-free-png.png',
                    (error, image) => {
                        if (error) throw error;

                        // Add the image to the map style.
                        map.addImage('incidenteImg', image);
                    })




                /*

                PREGUNTAR:

                COMO OPTIMIZAR CODIGO CON METODO PREFERIBLEMENTE

                TAMBIÉN SE PUEDE HACER CON UN SWITCH, PERO PREFIRO METODO



                */

                //AÑADIR MARCADORES PERSONALIZADOS MOSSOS
                map.addSource('point', {
                    'type': 'geojson',
                    'data': {
                        'type': 'FeatureCollection',
                        'features': me.localizaciones2
                    }
                });

                // Imprimir imagen mossos

                map.addLayer({
                    'id': 'points',
                    'type': 'symbol',
                    'source': 'point', // reference the data source

                    'layout': {
                        'icon-image': 'mossos', // reference the image
                        'icon-size': 0.15
                    }
                });


                //AÑADIR MARCADORES PERSONALIZADOS TRANSIT

                map.addSource('transitPoint', {
                    'type': 'geojson',
                    'data': {
                        'type': 'FeatureCollection',
                        'features': me.localizaciones1
                    }
                });

                // Imprimir imagen TRANSIT                
                map.addLayer({
                    'id': 'transits',
                    'type': 'symbol',
                    'source': 'transitPoint', // reference the data source

                    'layout': {
                        'icon-image': 'transit', // reference the image
                        'icon-size': 0.15
                    }
                });

                //AÑADIR MARCADORES PERSONALIZADOS BOMBERS

                map.addSource('bombersPoint', {
                    'type': 'geojson',
                    'data': {
                        'type': 'FeatureCollection',
                        'features': me.localizaciones3
                    }
                });

                // Imprimir imagen BOMBEROS               
                map.addLayer({
                    'id': 'bomber',
                    'type': 'symbol',
                    'source': 'bombersPoint', // reference the data source

                    'layout': {
                        'icon-image': 'bombers', // reference the image
                        'icon-size': 0.15
                    }
                });

                //AÑADIR MARCADORES PERSONALIZADOS POLICIA LOCAL

                map.addSource('pLocalPoint', {
                    'type': 'geojson',
                    'data': {
                        'type': 'FeatureCollection',
                        'features': me.localizaciones4
                    }
                });

                // Imprimir imagen POLICIA LOCAL             
                map.addLayer({
                    'id': 'pLocal',
                    'type': 'symbol',
                    'source': 'pLocalPoint', // reference the data source

                    'layout': {
                        'icon-image': 'pLocal', // reference the image
                        'icon-size': 0.15
                    }
                });

                //AÑADIR MARCADORES PERSONALIZADOS POLICIA MUNICIPAL

                map.addSource('pMunicipalPoint', {
                    'type': 'geojson',
                    'data': {
                        'type': 'FeatureCollection',
                        'features': me.localizaciones5
                    }
                });

                // Imprimir imagen POLICIA LOCAL             
                map.addLayer({
                    'id': 'pMunicipal',
                    'type': 'symbol',
                    'source': 'pMunicipalPoint', // reference the data source

                    'layout': {
                        'icon-image': 'pMunicipal', // reference the image
                        'icon-size': 0.15
                    }
                });


                //AÑADIR MARCADORES PERSONALIZADOS GUARDIA URBANA

                map.addSource('gUrbanaPointer', {
                    'type': 'geojson',
                    'data': {
                        'type': 'FeatureCollection',
                        'features': me.localizaciones6
                    }
                });

                // Imprimir imagen GUARDIA URBANA            
                map.addLayer({
                    'id': 'gUrbana',
                    'type': 'symbol',
                    'source': 'gUrbanaPointer', // reference the data source

                    'layout': {
                        'icon-image': 'gUrbana', // reference the image
                        'icon-size': 0.15
                    }
                });





                //AÑADIR MARCADOR PERSONALIZADO INCIDENCIA
                map.addSource('incidenciaPoint', {
                    'type': 'geojson',
                    'data': {
                        'type': 'FeatureCollection',
                        'features': me.incidentGeocoder
                    }
                });

                // Imprimir marcador INCIDENCIA                
                map.addLayer({
                    'id': 'incidencia',
                    'type': 'symbol',
                    'source': 'incidenciaPoint', // reference the data source
                    'layout': {
                        'icon-image': 'incidenteImg', // reference the image
                        'icon-size': 0.03
                    }
                });

            });


        },
        imprimirMapa() {
            mapboxgl.accessToken = this.accessToken;

            return new mapboxgl.Map({
                container: "mapContainer",
                style: "mapbox://styles/mapbox/streets-v11",
                center: [2.2001107, 41.406612],
                zoom: 12,
            });

        }
    },

    mounted() {
        this.sendVar = this.send
        //CUSTOM MARKER
        const marker = new mapboxgl.Marker({
            element: document.createElement('div'),
        });

        this.imprimirMapa()

    },

};


</script>
<style>
.basemap {
    width: 300px;
    height: 300px;
}
</style>